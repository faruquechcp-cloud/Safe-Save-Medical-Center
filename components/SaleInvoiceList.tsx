...
import { SaleInvoice, GenericSortConfig, SortableSaleInvoiceKeys } from '../types';
...
interface SaleInvoiceListProps {
  ...
  onSort: (key: SortableSaleInvoiceKeys) => void;
  sortConfig: GenericSortConfig<SortableSaleInvoiceKeys> | null;
}
...
const TableHeader: React.FC<{ 
    label: string; 
    sortKey: SortableSaleInvoiceKeys; 
    onSort: (key: SortableSaleInvoiceKeys) => void; 
    sortConfig: GenericSortConfig<SortableSaleInvoiceKeys> | null;
    className?: string;
...
const SaleInvoiceList: React.FC<SaleInvoiceListProps> = ({ invoices, onViewDetails, onEdit, onSort, sortConfig }) => {
    ...
    return (
      <div className="text-center py-10 bg-white rounded-lg shadow">
        <svg className="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.863 4.037A3.047 3.047 0 0112.91 7.084L16.5 10.674m-3.59 3.59L9.326 17.85A3.047 3.047 0 016.28 14.803L2.7 11.213m11.257-7.176a3.033 3.033 0 00-4.288 0L2.7 11.213a3.033 3.033 0 000 4.288l6.964 6.964a3.033 3.033 0 004.288 0L21.3 15.118a3.033 3.033 0 000-4.288L13.957 4.037z" />
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16.5 10.674l-3.59 3.59m0 0L16.5 10.674" />
        </svg>
        <h3 className="mt-2 text-sm font-medium text-gray-900">{t('saleInvoiceList.noInvoices', 'No sale invoices found.')}</h3>
        <p className="mt-1 text-sm text-gray-500">{t('saleInvoiceList.getStarted', 'Get started by creating a new sale invoice.')}</p>
      </div>
    );
  }
  ...
  const formatDate = (dateString: string) => {
    ...
    return new Date(dateString).toLocaleDateString(t('language.current') === 'bn' ? 'bn-BD' : 'en-CA', { year: 'numeric', month: 'short', day: 'numeric' });
  };

  const formatTime = (dateString: string) => {
    if (!dateString || dateString.length <= 10) return 'N/A';
    try {
      return new Date(dateString).toLocaleTimeString(t('language.current') === 'bn' ? 'bn-BD' : 'en-US', { 
        hour: '2-digit', 
        minute: '2-digit',
        hour12: true 
      });
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <TableHeader label={t('saleInvoiceForm.timeLabel', 'Time')} sortKey="date" onSort={onSort} sortConfig={sortConfig} />
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentItems.map((invoice) => (
              <tr key={invoice.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">{formatTime(invoice.date)}</td>
                <td className="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">{invoice.customerName || t('reports.walkInCustomer', 'Walk-in Customer')}</td>
                <td className="px-4 py-3 text-sm text-gray-500 whitespace-nowrap text-center">{(invoice.items || []).length}</td>
                <td className="px-4 py-3 text-sm text-gray-500 whitespace-nowrap text-right">Tk. {(Number(invoice.totalAmount) || 0).toFixed(2)}</td>
