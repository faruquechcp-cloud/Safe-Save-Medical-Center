...
import { SaleReturnInvoice, GenericSortConfig, SortableSaleReturnInvoiceKeys } from '../types';
...
interface SaleReturnInvoiceListProps {
  invoices: SaleReturnInvoice[];
  onViewDetails: (invoice: SaleReturnInvoice) => void; 
  onEdit: (invoice: SaleReturnInvoice) => void;
  onSort: (key: SortableSaleReturnInvoiceKeys) => void;
  sortConfig: GenericSortConfig<SortableSaleReturnInvoiceKeys> | null;
}
...
const TableHeader: React.FC<{ 
    label: string; 
    sortKey: SortableSaleReturnInvoiceKeys; 
    onSort: (key: SortableSaleReturnInvoiceKeys) => void; 
    sortConfig: GenericSortConfig<SortableSaleReturnInvoiceKeys> | null;
    className?: string;
...
const SaleReturnInvoiceList: React.FC<SaleReturnInvoiceListProps> = ({ invoices, onViewDetails, onEdit, onSort, sortConfig }) => {
  ...
  if (invoices.length === 0) {
    return (
      <div className="text-center py-10 bg-white rounded-lg shadow">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900">{t('saleReturnInvoiceList.noSaleReturns', 'No sale returns found.')}</h3>
        <p className="mt-1 text-sm text-gray-500">{t('saleReturnInvoiceList.getStarted', 'Create a new sale return to get started.')}</p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto flex-1">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <TableHeader label={t('saleReturnInvoiceForm.returnInvoiceNumberLabel', "Return Invoice #")} sortKey="returnInvoiceNumber" onSort={onSort} sortConfig={sortConfig} />
              ...
              <TableHeader label={t('saleReturnInvoiceForm.summary.totalRefund', "Total Refund")} sortKey="totalRefundAmount" onSort={onSort} sortConfig={sortConfig} className="text-right"/>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentItems.map((invoice) => (
              <tr key={invoice.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-sm text-gray-500 whitespace-nowrap text-right">Tk. {invoice.totalRefundAmount.toFixed(2)}</td>
...
export default SaleReturnInvoiceList;
