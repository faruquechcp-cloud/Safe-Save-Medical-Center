...
import { PurchaseInvoice, GenericSortConfig, SortablePurchaseInvoiceKeys } from '../types';
...
interface PurchaseInvoiceListProps {
  ...
  onSort: (key: SortablePurchaseInvoiceKeys) => void;
  sortConfig: GenericSortConfig<SortablePurchaseInvoiceKeys> | null;
}
...
const TableHeader: React.FC<{ 
    label: string; 
    sortKey: SortablePurchaseInvoiceKeys; 
    onSort: (key: SortablePurchaseInvoiceKeys) => void; 
    sortConfig: GenericSortConfig<SortablePurchaseInvoiceKeys> | null;
    className?: string;
...
const PurchaseInvoiceList: React.FC<PurchaseInvoiceListProps> = ({ invoices, onViewDetails, onEdit, onSort, sortConfig }) => {
  ...
  if (invoices.length === 0) {
    return (
      <div className="text-center py-10 bg-white rounded-lg shadow">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900">{t('purchaseInvoiceList.noInvoices', 'No purchase invoices found.')}</h3>
        <p className="mt-1 text-sm text-gray-500">{t('purchaseInvoiceList.getStarted', 'Get started by creating a new purchase invoice.')}</p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto flex-1">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <TableHeader label={t('purchaseInvoiceForm.invoiceNumberLabel', 'Invoice #')} sortKey="invoiceNumber" onSort={onSort} sortConfig={sortConfig} />
              <TableHeader label={t('purchaseInvoiceForm.dateLabel', 'Date')} sortKey="date" onSort={onSort} sortConfig={sortConfig} />
              <TableHeader label={t('purchaseInvoiceForm.supplierLabel', 'Supplier')} sortKey="supplierName" onSort={onSort} sortConfig={sortConfig} />
              <th scope="col" className="px-4 py-3.5 text-center text-sm font-semibold text-gray-900">{t('purchaseInvoiceForm.itemsTitle', 'Items')}</th>
              <TableHeader label={t('purchaseInvoiceForm.summary.netPayable', 'Total Amount')} sortKey="totalAmount" onSort={onSort} sortConfig={sortConfig} className="text-right"/>
